...
Além das páginas estáticas, o repositório hospeda um pequeno painel administrativo (`admin/`) para a escola trocar/adicionar, sem editar código: os **3 banners do topo** da home (cada um com versão desktop e mobile) e os **eventos da "Cobertura de Eventos"** (`saiba-mais.html`) — cada evento tem capa, título e descrição, e leva a uma página própria (`evento.html?id=<id>`) com todas as fotos daquele evento. Arquitetura, decisão de custo (R$ 0, sem serviços novos) e as tasks estão em [docs/sistema-de-imagens-plano.md](docs/sistema-de-imagens-plano.md); o guia de configuração (token do GitHub, senhas, variáveis de ambiente) está em [docs/sistema-de-imagens-setup.md](docs/sistema-de-imagens-setup.md).

- **`content/banners.json`** (cada item com `desktop`/`mobile`/`alt`/`link`) e **`content/eventos.json`** (array de eventos, cada um com `id`/`titulo`/`descricao`/`data`/`capa`/`fotos[]`) são a fonte de verdade dessas duas áreas. `index.html` e `saiba-mais.html` carregam `content/banners.json` e a lista de eventos via `js/content.js` (executa antes de `js/main.js`); `evento.html` carrega um evento específico via `js/evento.js`, lendo o `id` da query string. Todos com fallback silencioso — se a busca falhar, a página não quebra. O ponto de corte usado para escolher a imagem desktop/mobile do banner é `(max-width: 640px)`, o mesmo já usado em outros lugares do CSS.
- Cada evento grava suas imagens em `assets/img/eventos/<id-do-evento>/` (capa em `capa.jpg`, fotos com nome `<timestamp>-<índice>.jpg`) — `<id-do-evento>` é gerado a partir do título na criação do evento e não muda depois, mesmo que o título seja editado (evita órfãos). `api/publish.js` só aceita gravar dentro desse padrão de caminho (mais o legado `assets/img/eventos/*.jpg` da v1, ainda aceito por compatibilidade).
